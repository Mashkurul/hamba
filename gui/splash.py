# =============================================================
# gui/splash.py - Animated Splash Screen
# =============================================================
# Shows on app launch:
#   1. Window fades in
#   2. Cow logo scales up with a spring overshoot
#   3. "HAMBA" title slides up + fades in
#   4. Tagline fades in
#   5. Progress bar fills
#   6. Window fades out -> on_done() is called
# =============================================================
...
class SplashScreen(ctk.CTkToplevel):
    def __init__(self, on_done, width=460, height=420):
        ...
        # Animation state
        self._steps = None   # list of (delay_ms, func)
        self._t = 0
        self._run()

    # ----------------------------------------------------------
    def _build(self):
        # Decorative ring behind the cow
    ...
    # ----------------------------------------------------------
    # Animation driver: schedules the timeline
    # ----------------------------------------------------------
    def _run(self):
        self._steps = [
            (0,    self._fade_in),        # window alpha 0 -> 1
            (250,  self._logo_spring),    # logo scale up with overshoot
            (850,  self._title_in),       # title slides up
            (1050, self._tag_in),         # tagline fades in
            (1150, self._bar_fill),       # progress bar fills
            (2400, self._fade_out),       # window alpha 1 -> 0
        ]
        self._t = 0
        self._schedule_next()
    ...
    def _logo_spring(self):
        ...
        sizes = [28, 40, 56, 74, 92, 104, 96, 100, 96, 98]   # overshoot
    ...
    def _title_in(self):
        ...
        # Slide up: start lower, move to final position
        for i in range(1, 11):
            self.after(i * 18,
